...
void main() {
  ...
  vec3 colorComponent = clamp(color_GxMaterialColor12345 + clamp(texture(texture0, uv0).rgb*color_GxMaterialColor5*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor5), 0, 1), 0, 1), 0, 1);

  float alphaComponent = scalar_GxMaterialAlpha12345;

  fragColor = vec4(colorComponent, alphaComponent);
}
DiffuseColors[2].a + scalar_GxAmbientAlpha2);
