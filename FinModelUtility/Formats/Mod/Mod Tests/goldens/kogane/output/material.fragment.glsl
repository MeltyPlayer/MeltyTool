...
uniform float scalar_GxAmbientAlpha3;
...
void main() {
  ...
  vec3 colorComponent = clamp(texture(texture0, uv0).rgb*color_GxMaterialColor3*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor3), 0, 1) + vec3(-0.5) + color_GxColor0*texture(texture1, acos(normalUv) / 3.14159).rgb, 0, 1);

  float alphaComponent = scalar_GxMaterialAlpha3*(individualLightDiffuseColors[0].a + individualLightDiffuseColors[1].a + individualLightDiffuseColors[2].a + scalar_GxAmbientAlpha3);

  fragColor = vec4(colorComponent, alphaComponent);
}
 uv0).rgb*clamp((color_GxMaterialColor0*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor0), 0, 1) + vec3(1) + vec3(-1)*vec3(scalar_GxAlpha2) + texture(texture1, acos(normalUv) / 3.14159).rgb*vec3(scalar_GxAlpha2) + vec3(-1)*texture(texture1, acos(normalUv) / 3.14159).rgb*vec3(scalar_GxAlpha2)), 0, 1) + color_GxColor1*(vec3(1) + vec3(-1)*texture(texture1, acos(normalUv) / 3.14159).rgb) + color_GxColor0*texture(texture1, acos(normalUv) / 3.14159).rgb + vec3(-0.5), 0, 1);

  float alphaComponent = scalar_GxMaterialAlpha0*(individualLightDiffuseColors[0].a + individualLightDiffuseColors[1].a + individualLightDiffuseColors[2].a + scalar_GxAmbientAlpha0);
