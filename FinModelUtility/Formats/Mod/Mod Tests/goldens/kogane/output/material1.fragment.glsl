...
uniform float scalar_GxAmbientAlpha1;
...
void main() {
  ...
  vec3 colorComponent = clamp(texture(texture0, uv0).rgb*color_GxMaterialColor1*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor1), 0, 1) + vec3(-0.5) + color_GxColor0*texture(texture1, acos(normalUv) / 3.14159).rgb, 0, 1);

  float alphaComponent = scalar_GxMaterialAlpha1*(individualLightDiffuseColors[0].a + individualLightDiffuseColors[1].a + individualLightDiffuseColors[2].a + scalar_GxAmbientAlpha1);
