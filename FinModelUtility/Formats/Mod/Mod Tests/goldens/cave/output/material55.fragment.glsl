...
void main() {
  ...
  vec3 colorComponent = vertexColor0.rgb*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor55), 0, 1)*(vec3(1) + vec3(-1)*texture(texture0, uv0).rgb*vec3(0.5)*texture(texture1, uv0).rgb)*vec3(0.5);

  float alphaComponent = texture(texture0, uv0).a*vertexColor0.a*texture(texture1, uv0).a;
