...
uniform float scalar_GxMaterialAlpha55;
...
void main() {
  ...
  vec3 colorComponent = vertexColor0.rgb*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor55), 0, 1)*(vec3(1) + vec3(-1)*texture(texture0.sampler, texture0.transform2d * vec3((uv0).x, (uv0).y, 1)).rgb*vec3(0.5)*texture(texture1.sampler, texture1.transform2d * vec3((uv0).x, (uv0).y, 1)).rgb)*vec3(0.5);

  float alphaComponent = texture(texture0.sampler, texture0.transform2d * vec3((uv0).x, (uv0).y, 1)).a*scalar_GxMaterialAlpha55*texture(texture1.sampler, texture1.transform2d * vec3((uv0).x, (uv0).y, 1)).a;

  fragColor = vec4(colorComponent, alphaComponent);

  if (!(alphaComponent <= 0.19607843 || alphaComponent >= 0.47058824)) {
    discard;
  }
}
