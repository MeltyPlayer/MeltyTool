...
uniform vec3 color_GxMaterialColor32;
uniform vec3 color_GxAmbientColor32;
uniform float scalar_GxMaterialAlpha32;
uniform float scalar_GxAmbientAlpha32;
...
void main() {
  ...
  vec3 colorComponent = color_GxMaterialColor32*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor32), 0, 1)*(vec3(1) + vec3(-1)*texture(texture0, uv0).rgb*vec3(0.5)*texture(texture1, uv0).rgb)*vec3(0.5);

  float alphaComponent = texture(texture0, uv0).a*scalar_GxMaterialAlpha32*(individualLightDiffuseColors[0].a + individualLightDiffuseColors[1].a + individualLightDiffuseColors[2].a + scalar_GxAmbientAlpha32)*texture(texture1, uv0).a;

  fragColor = vec4(colorComponent, alphaComponent);

  if (!(fragColor.a <= 0.19607843 || fragColor.a >= 0.47058824)) {
    discard;
  }
}
