...
void main() {
  ...
  vec3 colorComponent = clamp(texture(texture0, uv0).rgb*vertexColor0.rgb*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor70), 0, 1), 0, 1);

  float alphaComponent = texture(texture0, uv0).a*scalar_GxMaterialAlpha70;
