...
uniform vec3 color_GxAmbientColor18;
uniform float scalar_GxMaterialAlpha18;
uniform float scalar_GxAmbientAlpha18;
...
void main() {
  ...
  vec3 colorComponent = texture(texture1, uv0).rgb*texture(texture0.sampler, texture0.transform2d * vec3((acos(normalUv) / 3.14159).x, (acos(normalUv) / 3.14159).y, 1)).rgb*color_GxMaterialColor18*clamp((individualLightDiffuseColors[0].rgb + individualLightDiffuseColors[1].rgb + individualLightDiffuseColors[2].rgb + color_GxAmbientColor18), 0, 1)*vec3(2);

  float alphaComponent = texture(texture0.sampler, texture0.transform2d * vec3((acos(normalUv) / 3.14159).x, (acos(normalUv) / 3.14159).y, 1)).a*scalar_GxMaterialAlpha18*(individualLightDiffuseColors[0].a + individualLightDiffuseColors[1].a + individualLightDiffuseColors[2].a + scalar_GxAmbientAlpha18);
