...
void main() {
  ...
  vec3 colorComponent = clamp(color_3dsColor2*mergedLightSpecularColor.rgb*vec3(0.498039215803) + clamp((ambientLightColor.rgb*vec3(0.800000011921) + mergedLightDiffuseColor.rgb)*clamp(clamp((color_3dsColor3 + texture(texture0, uv0).rgb), 0.0, 1.0)*texture(texture1, uv0).rgb, 0.0, 1.0), 0.0, 1.0), 0.0, 1.0);

  float alphaComponent = vertexColor0.a*texture(texture1, uv0).a*scalar_3dsAlpha1*scalar_3dsAlpha2;
