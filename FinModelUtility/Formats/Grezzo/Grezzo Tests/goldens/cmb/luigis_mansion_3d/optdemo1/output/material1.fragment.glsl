...
void main() {
  ...
  vec3 colorComponent = clamp(texture(texture0, uv0).rgb*mergedLightSpecularColor.rgb + clamp((ambientLightColor.rgb*vec3(0.800000011921) + mergedLightDiffuseColor.rgb)*texture(texture0, uv0).rgb, 0.0, 1.0), 0.0, 1.0);
