...
void main() {
  ...
  vec3 colorComponent = clamp(texture(texture1, uv0).rgb*mergedLightSpecularColor.rgb*vec3(2.0) + clamp((ambientLightColor.rgb*vec3(0.800000011921) + mergedLightDiffuseColor.rgb)*texture(texture1, uv0).rgb, 0.0, 1.0), 0.0, 1.0);
