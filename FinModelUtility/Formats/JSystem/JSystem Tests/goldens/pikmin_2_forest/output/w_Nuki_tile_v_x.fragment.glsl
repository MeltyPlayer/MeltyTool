...
void main() {
  ...
  vec3 colorComponent = clamp(clamp((clamp(vec3(texture(texture1, acos(linearReflectionUv) / 3.14159).a)*clamp(vertexColor0.rgb*clamp((individualLightDiffuseColors[0].rgb + color_GxAmbientColor0), 0, 1)*texture(texture0, uv0).rgb, 0, 1), 0, 1) + vertexColor0.rgb*clamp((individualLightDiffuseColors[0].rgb + color_GxAmbientColor0), 0, 1)*texture(texture2, uv0).rgb), 0, 1)*texture(texture3, uv1).rgb*vec3(2), 0, 1);
