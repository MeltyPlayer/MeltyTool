...
void main() {
  ...
  vec3 colorComponent = clamp(texture(texture1, uv1).rgb*clamp(texture(texture0, uv0).rgb*color_GxMaterialColor0*clamp((individualLightDiffuseColors[0].rgb + color_GxAmbientColor0), 0, 1), 0, 1)*vec3(2), 0, 1);
